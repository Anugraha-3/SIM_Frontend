...
const servicesData = {
  ...
  image: "/service/digital_marketing.jpg",
  ...
  image: "/service/product_promotion.jpg",
  ...
  image: "/service/bulk_service.jpg",
...
const contactData = {
  ...
  mail: {
    icon: "mail",
    label: "Email",
    action: () => window.location.href = "mailto:admin@sunnetwork.info",
    color: "from-red-500 to-red-600"
  },
  phone: {
    icon: "phone",
    label: "Phone",
    action: () => window.location.href = "tel:+917788003366",
...
export default function DigitalMarketingServicesPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans overflow-x-hidden">
      {/* Enhanced Service Selector - Mobile Responsive */}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            <button
              ...
              className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-white/10 backdrop-blur-md rounded-lg border border-white/20 hover:bg-white/20 transition-all duration-300 hover:scale-105"
            >
              <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5" />
              <span className="hidden sm:inline text-sm sm:text-base">Back to Services</span>
              <span className="sm:hidden text-sm">Back</span>
            </button>

            {/* Service Dropdown - Mobile Responsive */}
            <div className="relative flex-1 max-w-xs sm:max-w-none sm:flex-none mx-2 sm:mx-0">
              <button
                onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                className="flex items-center gap-2 sm:gap-3 px-3 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-pink-500/20 to-indigo-500/20 backdrop-blur-md rounded-xl border border-white/20 hover:from-pink-500/30 hover:to-indigo-500/30 transition-all duration-300 w-full sm:w-auto"
              >
                <div className="p-1 sm:p-2 bg-gradient-to-r from-pink-500 to-indigo-500 rounded-lg flex-shrink-0">
                  {serviceData.icon}
                </div>
                <div className="text-left flex-1 min-w-0">
                  <div className="font-semibold text-white text-sm sm:text-base truncate">{serviceData.title}</div>
                  <div className="text-xs text-gray-400 hidden sm:block">{currentIndex + 1} of {servicesArray.length}</div>
                </div>
                <ChevronDown className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform duration-300 flex-shrink-0 ${isDropdownOpen ? 'rotate-180' : ''}`} />
              </button>

              {/* Dropdown Menu - Mobile Responsive */}
              {isDropdownOpen && (
                <div className="absolute top-full mt-2 left-0 right-0 sm:right-0 sm:left-auto sm:w-80 bg-gray-900/95 backdrop-blur-xl rounded-xl border border-white/20 shadow-2xl overflow-hidden animate-in slide-in-from-top-2 duration-200 z-50">
                  {servicesArray.map((service, index) => (
                    <button
                      key={service.id}
                      onClick={() => handleServiceChange(service.id)}
                      className={`w-full p-3 sm:p-4 text-left hover:bg-white/10 transition-all duration-200 border-b border-white/5 last:border-b-0 ${
                        currentServiceId === service.id ? 'bg-gradient-to-r from-pink-500/20 to-indigo-500/20' : ''
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg flex-shrink-0 ${currentServiceId === service.id ? 'bg-gradient-to-r from-pink-500 to-indigo-500' : 'bg-white/10'}`}>
                          {service.icon}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-white text-sm sm:text-base">{service.title}</div>
                          <div className="text-xs sm:text-sm text-gray-400 line-clamp-1">{service.description}</div>
                        </div>
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              <button
                onClick={prevService}
                className="p-1.5 sm:p-2 bg-white/10 hover:bg-white/20 rounded-lg transition-all duration-300 hover:scale-110"
              ...
              <button
                onClick={nextService}
                className="p-1.5 sm:p-2 bg-white/10 hover:bg-white/20 rounded-lg transition-all duration-300 hover:scale-110 rotate-180"
                title="Next Service"
              >
                <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5" />
              </button>
      ...
      <div className={`relative h-[50vh] sm:h-[60vh] lg:h-[70vh] overflow-hidden transition-all duration-500 ${isAnimating ? 'opacity-50 scale-95' : 'opacity-100 scale-100'}`}>
        <div
          ...
          <div className="max-w-6xl mx-auto">
            <div className={`transform transition-all duration-700 ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
              <div className="flex items-center gap-3 sm:gap-4 lg:gap-6 mb-4 sm:mb-6">
                <div className="p-2 sm:p-3 lg:p-4 bg-gradient-to-r from-pink-500 to-indigo-500 rounded-xl sm:rounded-2xl shadow-2xl animate-pulse flex-shrink-0">
                  {serviceData.icon}
                </div>
                <div className="min-w-0 flex-1">
                  <h1 className="text-2xl sm:text-4xl md:text-5xl lg:text-7xl font-bold bg-gradient-to-r from-white via-gray-100 to-white bg-clip-text text-transparent leading-tight">
                    {serviceData.title}
                  </h1>
        ...
        <div className="absolute top-20 right-20 w-20 h-20 bg-gradient-to-r from-pink-500/20 to-indigo-500/20 rounded-full blur-xl animate-bounce hidden sm:block"></div>
        <div className="absolute top-40 left-20 w-16 h-16 bg-gradient-to-r from-indigo-500/20 to-purple-500/20 rounded-full blur-xl animate-bounce delay-75 hidden sm:block"></div>
      </div>

      {/* Enhanced Content with Stagger Animations - Mobile Responsive */}
      <div className={`max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16 transition-all duration-500 ${isAnimating ? 'opacity-50' : 'opacity-100'}`}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16">
          {/* Enhanced Details - Mobile Responsive */}
          <div className={`transform transition-all duration-700 delay-100 ${mounted ? 'translate-x-0 opacity-100' : '-translate-x-8 opacity-0'}`}>
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-6 sm:mb-8 bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 bg-clip-text text-transparent">
              About This Service
            </h2>
            <div className="relative">
              <div className="absolute -left-4 top-0 w-1 h-full bg-gradient-to-b from-pink-500 to-indigo-500 rounded-full hidden sm:block"></div>
              <p className="text-gray-300 text-base sm:text-lg leading-relaxed mb-6 sm:mb-8 sm:pl-8">
                {serviceData.details}
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center gap-4 sm:gap-6">
              <button 
                ...
                className="group px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-pink-500 to-indigo-500 rounded-xl font-semibold hover:shadow-2xl hover:shadow-pink-500/25 transition-all duration-300 hover:scale-105 hover:-translate-y-1 w-full sm:w-auto"
              >
          ...
          <div className={`transform transition-all duration-700 delay-200 ${mounted ? 'translate-x-0 opacity-100' : 'translate-x-8 opacity-0'}`}>
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-6 sm:mb-8 text-white">Key Features</h2>
            <div className="grid grid-cols-1 gap-3 sm:gap-4">
              {serviceData.features.map((feature) => (
                <div
                  key={feature}
                  className={`group p-4 sm:p-5 bg-white/5 border border-white/10 rounded-xl backdrop-blur-md hover:bg-white/10 hover:border-white/20 transition-all duration-300 hover:scale-105 hover:-translate-y-1 transform ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}
                  style={{ transitionDelay: `${300}ms` }}
                >
                  <div className="flex items-center gap-3 sm:gap-4">
                    <div className="p-2 bg-gradient-to-r from-pink-500 to-indigo-500 rounded-lg group-hover:scale-110 transition-transform duration-300 flex-shrink-0">
        ...
        {/* Enhanced Additional Content - Mobile Responsive */}
        <div className={`mt-12 sm:mt-16 lg:mt-20 p-6 sm:p-8 bg-gradient-to-r from-pink-500/10 via-purple-500/10 to-indigo-500/10 rounded-2xl sm:rounded-3xl border border-white/10 backdrop-blur-md transform transition-all duration-700 delay-300 ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          <h3 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 bg-gradient-to-r from-pink-500 to-indigo-500 bg-clip-text text-transparent">
        ...
        {/* Enhanced Service Navigation - Mobile Responsive */}
        <div className={`mt-12 sm:mt-16 flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-0 p-4 sm:p-6 bg-white/5 rounded-2xl backdrop-blur-md border border-white/10 transform transition-all duration-700 delay-400 ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          <div className="flex items-center gap-4 w-full sm:w-auto">
            <div className="text-sm text-gray-400">
              Service {currentIndex + 1} of {servicesArray.length}
            </div>
            <div className="flex-1 sm:w-32 bg-white/10 rounded-full h-2 overflow-hidden">
              <div 
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-1/4 -left-20 w-40 h-40 bg-pink-500/5 rounded-full blur-3xl animate-pulse hidden sm:block"></div>
        <div className="absolute bottom-1/4 -right-20 w-60 h-60 bg-indigo-500/5 rounded-full blur-3xl animate-pulse delay-75 hidden sm:block"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-purple-500/3 rounded-full blur-3xl animate-pulse delay-150 hidden lg:block"></div>
      </div>

      {/* Contact Popup - Mobile Responsive */}
      {showContactPopup && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-400">
          <div className="relative bg-gray-900/95 backdrop-blur-xl rounded-2xl sm:rounded-3xl border border-white/20 p-6 sm:p-8 lg:p-10 max-w-xl w-full shadow-2xl animate-in zoom-in-90 duration-400 overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <button
              ...
              className="absolute top-4 right-4 sm:top-5 sm:right-5 p-2 text-gray-400 hover:text-white hover:bg-white/10 rounded-full transition-all duration-300 hover:rotate-180 hover:scale-110 z-10"
            >
              <X className="w-4 h-4" />
            </button>
            ...
            <div className="text-center mb-8 sm:mb-10 relative">
              <div className="flex justify-center mb-4 sm:mb-5">
                <div className="relative">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-r from-pink-500 to-indigo-500 rounded-xl sm:rounded-2xl animate-ping opacity-20"></div>
                  <div className="relative p-3 sm:p-4 bg-gradient-to-r from-pink-500 to-indigo-500 rounded-xl sm:rounded-2xl animate-in zoom-in duration-500 delay-200">
                    {serviceData.icon}
                  </div>
                  
                  {/* Orbiting particles - Hidden on mobile */}
                  <div className="absolute -inset-6 animate-spin hidden sm:block" style={{ animationDuration: '8s' }}>
                  ...
                  <div className="absolute -inset-6 animate-spin hidden sm:block" style={{ animationDuration: '12s', animationDirection: 'reverse' }}>
            ...
            {/* Enhanced Contact Options - Mobile Responsive */}
            <div className="grid grid-cols-2 sm:flex sm:justify-center gap-4 sm:gap-6 mb-6 sm:mb-8">
                ...
                return (
                  <div key={key} className="relative">
                    <button
                      ...
                      className={`group relative flex flex-col items-center gap-2 sm:gap-3 p-4 sm:p-5 bg-white/5 border border-white/10 rounded-xl sm:rounded-2xl hover:bg-white/10 transition-all duration-400 hover:scale-110 hover:-translate-y-2 animate-in slide-in-from-bottom-6 duration-500 hover:shadow-xl ${colors[key]} w-full`}
                      style={{ animationDelay: `${600 + index * 150}ms` }}
                    >
                      {/* Hover glow effect - Hidden on mobile for performance */}
                      <div className="absolute inset-0 rounded-xl sm:rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-400 bg-gradient-to-r from-white/5 to-white/10 blur-sm hidden sm:block"></div>
                      
                      <div className="relative">
                        {/* Icon background with animation */}
                        <div className="absolute inset-0 bg-white/10 rounded-lg sm:rounded-xl group-hover:scale-125 transition-transform duration-400 group-hover:rotate-12 hidden sm:block"></div>
                        <div className="relative p-2 sm:p-3 rounded-lg sm:rounded-xl">
                          <IconComponent className={`w-5 h-5 sm:w-6 sm:h-6 text-gray-300 transition-all duration-400 group-hover:scale-110 ${iconColors[key]}`} />
                        </div>
                      </div>
                      
                      <span className={`text-xs sm:text-sm font-medium text-gray-400 transition-all duration-400 group-hover:text-white text-center`}>
                        {contact.label}
                      </span>

                      {/* Ripple effect on click */}
                      <div className="absolute inset-0 rounded-xl sm:rounded-2xl opacity-0 group-active:opacity-100 bg-white/20 transition-opacity duration-150"></div>
                    </button>

                    {/* Connection dots - Hidden on mobile */}
                    {index < Object.keys(contactData).length - 1 && (
                      <div className="absolute top-1/2 -right-3 transform -translate-y-1/2 hidden sm:block">
            ...
            <div className="text-center animate-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: '1000ms' }}>
              ...
              <div className="flex justify-center gap-2 sm:gap-4 mt-4">
                <div className="flex items-center gap-1">
                  <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-pink-500/60 rounded-full animate-bounce"></div>
                  <span className="text-xs text-gray-500">Fast</span>
                </div>
                <div className="w-px h-3 sm:h-4 bg-white/10"></div>
                <div className="flex items-center gap-1">
                  <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-indigo-500/60 rounded-full animate-bounce delay-75"></div>
                  <span className="text-xs text-gray-500">Reliable</span>
                </div>
                <div className="w-px h-3 sm:h-4 bg-white/10"></div>
                <div className="flex items-center gap-1">
                  <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-purple-500/60 rounded-full animate-bounce delay-150"></div>
